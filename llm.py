...
with open('hg.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
print(len(vocab), "different tokens")
print(sum(p.numel() for p in m.parameters())/1e6, 'M parameters')

# create a PyTorch optimizer
optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate)
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, learning_rate, max_iters)

for iter in trange(max_iters):
    # every once in a while evaluate the loss on train and val sets
    if iter % eval_interval == 0 or iter == max_iters - 1:
        losses = estimate_loss()
        losscount.append(str(losses['train'].item()))
        losscount.append(str(losses['val'].item()))
...
open('loss2.txt','w').write('\n'.join(losscount))
# generate from the model
context = torch.zeros((1, 1), dtype=torch.long, device=device)
print(vocab.decode(m.generate(context, max_new_tokens=50)[0].tolist()))
# open('more.txt', 'w').write(vocab.decode(m.generate(context, max_new_tokens=10000)[0].tolist()))
device = torch.device('cpu')
model = model.to(device)
torch.save(model.state_dict(), 'hg1.pt')
torch.onnx.export(
    model.cpu(),
    torch.zeros((1, block_size), dtype=torch.long),
    'hg1.onnx',
    export_params=True,
    opset_version=17,
